...
include('../includes/connection.php');
       $sql="SELECT * FROM promted_products WHERE status='Pending'";
$result=$conn->query($sql);
if($result->num_rows>0){
	$counter = 1;
    while($row=$result->fetch_assoc()){
       $sid=$row["product_id"];
       $sql1= "SELECT ProductName FROM tblproducts WHERE id='$sid'";
...
{
$s=$_POST['id'];
$sus="UPDATE `tblproducts` SET `promoted`='Yes' WHERE `id`='$s'";
$query=mysqli_query($conn,$sus);
if($conn->query($sus)===TRUE);
{
$s=$_POST['id'];
$p="UPDATE `promted_products` SET `status`='Accepted' WHERE `product_id`='$s'";
$query=mysqli_query($conn,$p);
echo "<script>alert('Accepted & Product Promoted Successfully!'); window.location='manage-featured-ads.php'</script>";
}}?>
 <?php 
if(isset($_POST['decline']))
{
$s=$_POST['id'];
$dec="UPDATE `promted_products` SET `status`='Declined' WHERE `product_id`='$s'";
$query=mysqli_query($conn,$dec);
if($conn->query($dec)===TRUE);
{
echo "<script>alert('Request Declined!'); window.location='apromotion_requests.php'</script>";
}}?>                                   </tbody>
                                                </table>
...
<?php
include('../includes/connection.php');
       $sql="SELECT * FROM promted_products WHERE status='Accepted'";
$result=$conn->query($sql);
if($result->num_rows>0){
	$counter = 1;
    while($row=$result->fetch_assoc()){
...
$sus="UPDATE `tblproducts` SET `promoted`='No' WHERE `id`='$s'";
$query=mysqli_query($conn,$sus);
if($conn->query($sus)===TRUE);
{
echo "<script>alert('Request Declined!'); window.location='apromotion_requests.php'</script>";
$s=$_POST['id'];
$p="UPDATE `promted_products` SET `status`='Declined' WHERE `product_id`='$s'";
$query=mysqli_query($conn,$p);
}}?>                                   </tbody>
                                                </table>
...
<?php
include('../includes/connection.php');
       $sql="SELECT * FROM promted_products WHERE status='Declined'";
$result=$conn->query($sql);
if($result->num_rows>0){
	$counter = 1;
    while($row=$result->fetch_assoc()){
...
if($conn->query($sus)===TRUE);
{
echo "<script>alert('Request Accepted!'); window.location='apromotion_requests.php'</script>";
$s=$_POST['id'];
$p="UPDATE `promted_products` SET `status`='Accepted' WHERE `product_id`='$s'";
$query=mysqli_query($conn,$p);
}}?>                                   </tbody>
