...
include('../includes/connection.php');
       $sql="SELECT * FROM activate_shop WHERE status='Pending'";
$result=$conn->query($sql);
if($result->num_rows>0){
	$counter = 1;
    while($row=$result->fetch_assoc()){
       $sid=$row["shop_id"];
       $sql1= "SELECT Business FROM tblbusiness WHERE id='$sid'";
...
$new_date=date('Y-m-d',strtotime($payment_due. '+ 30 days'));
$sus="UPDATE `tblbusiness` SET `state`='Active', `payment_due`='$new_date' WHERE `id`='$s'";
$query=mysqli_query($conn,$sus);
if($conn->query($sus)===TRUE);
{
$s=$_POST['id'];
$p="UPDATE `activate_shop` SET `status`='Accepted' WHERE `shop_id`='$s'";
$query=mysqli_query($conn,$p);
echo "<script>alert('Accepted & Business Activated Successfully!'); window.location='manage-inactive-stores.php'</script>";
}}?>
 <?php 
if(isset($_POST['decline']))
{
$s=$_POST['id'];
$dec="UPDATE `activate_shop` SET `status`='Declined' WHERE `shop_id`='$s'";
...
<?php
include('../includes/connection.php');
       $sql="SELECT * FROM activate_shop WHERE status='Accepted'";
$result=$conn->query($sql);
if($result->num_rows>0){
	$counter = 1;
    while($row=$result->fetch_assoc()){
...
$payment_due=$row["payment_due"];
$new_date=date('Y-m-d',strtotime($payment_due. '- 30 days'));
$sus="UPDATE `tblbusiness` SET `state`='Suspended', `payment_due`='$new_date' WHERE `id`='$s'";
$query=mysqli_query($conn,$sus);
if($conn->query($sus)===TRUE);
{
echo "<script>alert('Request Declined!'); window.location='activation_requests.php'</script>";
$s=$_POST['id'];
$p="UPDATE `activate_shop` SET `status`='Declined' WHERE `shop_id`='$s'";
$query=mysqli_query($conn,$p);
}}?>                                   </tbody>
                                                </table>
...
<?php
include('../includes/connection.php');
       $sql="SELECT * FROM activate_shop WHERE status='Declined'";
$result=$conn->query($sql);
if($result->num_rows>0){
	$counter = 1;
    while($row=$result->fetch_assoc()){
...
$payment_due=$row["payment_due"];
$new_date=date('Y-m-d',strtotime($payment_due. '+ 30 days'));
$sus="UPDATE `tblshops` SET `state`='Active', `payment_due`='$new_date' WHERE `id`='$s'";
$query=mysqli_query($conn,$sus);
if($conn->query($sus)===TRUE);
{
echo "<script>alert('Request Accepted!'); window.location='activation_requests.php'</script>";
$s=$_POST['id'];
$p="UPDATE `activate_shop` SET `status`='Accepted' WHERE `shop_id`='$s'";
$query=mysqli_query($conn,$p);
}}?>                                   </tbody>
